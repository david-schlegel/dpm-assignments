...
- there is one value negativ in openness, but it is close to zero and only between o7 and o10. Decision, still correct. 
...
dat_bfi_scores_completness <- dat_bfi |>
  rowwise() |>
...
dat_bfi_participant_completeness <- dat_bfi |>  
  check_bfi_scale(sub_bfi_a, 9, "sub_bfi_a") |>  
  check_bfi_scale(sub_bfi_c, 9, "sub_bfi_c") |>
  check_bfi_scale(sub_bfi_e, 8, "sub_bfi_e") |>
  check_bfi_scale(sub_bfi_o, 10, "sub_bfi_o") |> 
  check_bfi_scale(sub_bfi_n, 8, "sub_bfi_n") |> 
...
dat_bfi_means <- dat_bfi |> 
  mutate(mean_bfi_a = rowMeans(select(dat_bfi, all_of(sub_bfi_a))),
         mean_bfi_c = rowMeans(select(dat_bfi, all_of(sub_bfi_c))), 
         mean_bfi_e = rowMeans(select(dat_bfi, all_of(sub_bfi_e))), 
         mean_bfi_n = rowMeans(select(dat_bfi, all_of(sub_bfi_o))),
         mean_bfi_o = rowMeans(select(dat_bfi, all_of(sub_bfi_n)))) |> 
...
data_processed_before_exlusions <- dat_bfi_scores_completness |> 
  ...
  full_join(dat_bfi, by = "unique_id") |> 
